...
//1) needs of input greater than 1 points, otherwise the interpolation is not defined
//2) sort the input vector and check if the extremes are -1 and 1 if not append them (done in .hh)
//3) check extremes of the vector, they have to be sufficiently different from -1 and 1 and in between
//Since here we have a vector with values oredered from -1 to 1, so a standard grid
//4)allocate memory and compute weights lambdaj
//5) compute the derivative matrix Dij
//6) implement the interpolation method with switch for the strategy to be used: naive, FBF, SBF
//7) poli_weight for the three strategies
//8) implement the derivative interpolation method with switch for the strategy 
//9) poli_weight_der for the three strategies
